...
import { CryptoConfig as CryptoConfigType } from '@aiofc/config';
...
export class CryptoService {
  ...
  private encryptAES(data: string, config?: Partial<AESConfig>): string {
    ...
    const iv = useRandomIV ? randomBytes(16) : Buffer.from(this.config.aes.iv);
    const key = Buffer.from(config?.key || this.config.aes.key);
  ...
  private decryptAES(
    ...
    if (useRandomIV) {
      ...
      iv = Buffer.from(this.config.aes.iv);
  ...
  private encryptRSA(data: string, config?: Partial<RSAConfig>): string {
    const publicKey = config?.publicKey || this.config.rsa.publicKey;
    if (!publicKey) {
  ...
  private decryptRSA(
    ...
    const privateKey = config?.privateKey || this.config.rsa.privateKey;
    if (!privateKey) {
